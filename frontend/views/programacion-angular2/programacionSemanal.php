...
<div class="panel panel-default">
    ...
    <table ng-table fixed-table-headers="semanal" class="table table-striped table-bordered table-hover">
        ...
        <tbody style="font-size: 10pt">
            ...
            <?php for($x=1;$x<=4;$x++): ?>
                <?php $class = $x % 2 != 0 ?'par' : 'impar'; ?>
                <td class="<?=$class?>"><input style="width: 50px;" ng-model-options="{updateOn: 'blur'}" ng-change="saveProgramacionSemanal(<?=$x?>);" ng-model="programacion.Prioridad<?=$x?>" value="{{programacion.Prioridad<?=$x?>}}"></td>
                <td class="<?=$class?>"><input style="width: 50px;" ng-model-options="{updateOn: 'blur'}" ng-change="saveProgramacionSemanal(<?=$x?>);" ng-model="programacion.Programadas<?=$x?>" value="{{programacion.Programadas<?=$x?>}}"></td>
