...
class ReportesController extends Controller
{
    ...
    public function actionDataTiemposMuertos(){
        ...
        $dataProvider = VTiemposMuertos::find()->where("IdArea = $IdArea $whereFecha")->orderBy('Inicio ASC')->asArray()->all();
       
        foreach ($dataProvider as &$value) {
            if($filtro == 'true'){
                $value['Inicio'] = strtotime(date('H:i',strtotime($value['Inicio']))) < strtotime('07:00') ? (date('Y-m-d',strtotime($value['Inicio']))." 07:00") : $value['Inicio'];
                $value['Inicio'] = strtotime(date('H:i',strtotime($value['Inicio']))) > strtotime('17:00') ? (date('Y-m-d',strtotime($value['Inicio']))." 17:00") : $value['Inicio'];
                
                $value['Fin'] = strtotime(date('H:i',strtotime($value['Fin']))) > strtotime('17:00') ? (date('Y-m-d',strtotime($value['Fin']))." 17:00") : $value['Fin'];
                $value['Fin'] = strtotime(date('H:i',strtotime($value['Fin']))) < strtotime('07:00') ? (date('Y-m-d',strtotime($value['Fin']))." 07:00") : $value['Fin'];
            }
    ...
    public function actionDataEte(){
        ...
        $where = "IdSubProceso = $IdSubProceso AND IdArea = $IdArea";
    ...
    public function actionEte(){
        ...
        $datos_ete = $model->getDatos($maquina,$ini,$fin, $this->areas->getCurrent(),$_GET['subProceso']);    
        $ResumenSem = array();
        $Totales = array();
        foreach ($datos_ete as &$key) {
            ...
            $Ti = strtotime(date('G:i',strtotime($key['Inicio'])))/ 60 ;
            $Tf = strtotime(date('G:i',strtotime($key['Fin']))) / 60 ;
