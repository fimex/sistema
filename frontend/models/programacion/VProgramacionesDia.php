...
class VProgramacionesDia extends \yii\db\ActiveRecord
{
    ...
    public function getDetallesDia($get){
        $command = \Yii::$app->db;
        $result = $command->createCommand("SELECT * FROM v_ProgramacionesDia 
                                                WHERE IdProducto = ".$get['IdProducto']." AND
                                                      Dia = '".$get['Dia']."' AND 
                                                      IdSubProceso = ".$get['IdSubProceso']." AND 
                                                      IdArea = ".$get['IdArea']." AND 
                                                      IdMaquina = ".$get['IdMaquina']." 
                                        ")->queryAll();
